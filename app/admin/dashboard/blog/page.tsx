...
function SuccessScreen({ slug, onReset }: { slug: string; onReset: () => void }) {
  return (
    <div className="p-8 flex flex-col items-center justify-center min-h-[60vh] text-center">
      ...
      <p className="text-white/40 font-mono text-sm mb-6">/blog/{slug}</p>
      ...
      <div className="rounded-2xl border border-white/6 bg-[#0d1117] overflow-hidden">
        ...
        {editorTab === "write" ? (
          <textarea value={form.content} onChange={set("content")} rows={26}
            ...
            className="w-full bg-transparent p-5 text-sm font-mono text-white/70 placeholder:text-white/15 focus:outline-none resize-none leading-7" />
        ) : (
          <div className="p-5 min-h-[400px]">
            <pre className="text-sm font-mono text-white/50 whitespace-pre-wrap leading-7">{form.content || "Nothing to preview."}</pre>
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className="rounded-2xl border border-white/6 bg-[#0d1117] overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/6 bg-[#161b22]">
              <ClipboardPaste className="w-3.5 h-3.5 text-white/30" />
              <span className="text-xs font-mono text-white/30">Paste full .mdx file (frontmatter + content)</span>
            </div>
            <textarea
              value={raw} onChange={(e) => setRaw(e.target.value)}
              rows={28}
              placeholder={`---\ntitle: "Your Title"\ndescription: "Description"\ndate: "2026-06-01"\ntags: ["Node.js", "TypeScript"]\nfeatured: false\nexcerpt: "Excerpt text"\n---\n\nYour MDX content here...`}
              className="w-full bg-transparent p-5 text-sm font-mono text-white/70 placeholder:text-white/20 focus:outline-none resize-none leading-7"
            />
          </div>
        ...
        <>
          ...
          <div className="p-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 space-y-3">
            ...
            {([
              ...
              <div key={k} className="flex gap-3 text-sm">
                <span className="w-24 text-white/30 font-mono flex-shrink-0">{k}</span>
                <span className="text-white/65 font-mono break-all">{v || <span className="text-white/20 italic">empty</span>}</span>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-white/6 bg-[#0d1117] overflow-hidden">
            <div className="px-4 py-2.5 border-b border-white/6 bg-[#161b22] flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-white/25">Content ({parsed.content.split(/\s+/).length} words)</span>
            </div>
            <pre className="p-5 text-xs font-mono text-white/40 whitespace-pre-wrap leading-6 max-h-60 overflow-auto">
    ...
    <div className="p-8 max-w-3xl">
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-white/35 font-mono">Create a post manually or paste a complete MDX file</p>
        </div>
      </div>
        ...
        {([
          ...
          <button key={id} onClick={() => setMode(id)}
            className={cn(
              ...
              mode === id ? "bg-white/8 text-white/85 shadow-sm" : "text-white/35 hover:text-white/60",
            )}>
            <Icon className="w-3.5 h-3.5" />{label}
          </button>
        ))}
