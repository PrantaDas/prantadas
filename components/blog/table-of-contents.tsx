...
export function TableOfContents({
  ...
  return (
    <nav aria-label="Table of contents" className="flex flex-col min-h-0 flex-1">
      ...
      <p className="text-xs font-mono text-white/68 uppercase tracking-widest mb-3 flex-shrink-0">
        On this page
      </p>
        ...
        {items.map((item) => (
          <a
            ...
            className={cn(
              ...
              active === item.id
                ? "text-primary"
                : "text-white/72 hover:text-white/90",
            )}
            onClick={(e) => {
              e.preventDefault();
      ...
      <div className="pt-4 mt-4 border-t border-white/6 flex-shrink-0">
        <a
          href="#comments"
          className="flex items-center gap-2 text-sm text-white/72 hover:text-primary transition-colors py-0.5"
          onClick={(e) => {
            e.preventDefault();
